...
const HELP_TEXT = `
    ${ BOT_PREFIX } ${ BOT_COMMANDS.IDENTIFY } - look up your SURE.social identity and update roles.
    ${ BOT_PREFIX } ${ BOT_COMMANDS.FORGET } - forget your SURE.social identity and remove roles.
...
class Identibot {
    ...
    async identify ( message, addressOrMagic ) {
        ...
        if ( components.length === 3 ) {
            ...
            if ( community !== components [ 0 ]) {
                message.reply ( `That community name did't match what I was expecting. It should be '${ community }'.` );
                return;
            }

            if ( username !== components [ 1 ]) {
                message.reply ( `That username did't match what I was expecting. It should be '${ username }'.` );
                return;
            }
    ...
    async onMessage ( message ) {
        ...
        if ( !command ) {
            message.reply ( `I need a command, an identity tag or a magic number.` );
            return;
        }
        ...
        switch ( command ) {
            ...
            case BOT_COMMANDS.FORGET: {
                ...
                message.reply ( 'OK, I removed all your SURE.social roles.' );
                break;
            }
